...
function Map(numPlayer,props) {
    ...
    this.initialize = function() {
        ...
        if(this.props.numKeys > 0) {
            ...
            this.blocks[iblk][jblk].above = [ new Tile(this.itemData["key"], iblk, jblk), this.mkBlock['block'](this, iblk, jblk)];
        }
    ...
    this.update = function(game) {
        ...
        function bombLimitCheck(player) {
            var count = 0;
            for(var i = 0; i < that.allBombs.length; i++) {
                if(that.allBombs[i].props.player === player) {
                    count++;
                }
            }
            return (count < game_configs["bombLimit"]);
        }
        ...
        if(game.map.player1.lives < 0 || game.map.player2.lives < 0) {
            eventHandlers["acceptKeysOff"]();
				    runScores();
