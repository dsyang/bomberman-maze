        ...
        function removeFireBlock(dir) {
            ...
            while (currTile.props.type != "wall" && stepCount > 0){
             ...
             //   var burntBomb = burnt && lastIndex >= 1 && (currTile.above[lastIndex-1].props.type === "bomb");
              /*  var aboveLen = currTile.above.length;
                var lastFire = false;
                while (!lastFire) {
                    board[pos.i][pos.j].above.pop();
                    aboveLen -= 1;
                    if (aboveLen == 0 || currTile.above[aboveLen-1].type !== "fire")
                        break;
                } */

                var printType = function(list) {
                    var typeList = [];
                    for(var i = 0; i < list.length; i++) {
                        typeList[i] = list[i].props.type;
                    }
                    return typeList;
                }
                ...
                if (burnt) {
                   ...
                   console.log(printType(board[pos.i][pos.j].above))
