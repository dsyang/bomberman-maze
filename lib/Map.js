...
function Map(numPlayer,props) {
    ...
    this.initialize = function() {
        ...
        if(this.props.keyNum > 0) {
            var iblk = this.inBlocks(game_configs["mapWidth"])-3;
            var jblk = this.inBlocks(game_configs["mapHeight"])-2;
    ...
    this.update = function(game) {
        ...
        function collectItem(playerPos,player) {
            ...
            if( result["success"] && result.value.props.type === "key") {
                ...
                if(player === 2) game.map.player1.objects.keyCount++;
            }
        };
